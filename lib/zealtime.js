(() => {
  ...
  function render() {
    ...
    document.querySelectorAll("[zid]").forEach((el) => {
      ...
      el.querySelectorAll("*").forEach((el2) => {
        if (el2.closest("[zid]").getAttribute("zid") != zid && el2.getAttribute("zid") != zid || el2.closest("[z]") !== null) return;
  ...
  z.internals = {
    // Shouldn't be used externally, exposed for specific use cases
    renderElement,
    render,
    stateListeners,
    modifications,
